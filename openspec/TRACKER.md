...
**Acceptance:**
- [ ] Sign-in screen matches R1.1–R1.3 (logo, welcome, copyright, tooltip, loading, errors)
- [ ] Role Selection works for dual-role users
- [ ] Forgot-password (SMTP + Admin-approval) works
- [ ] Lockout after 5/10min; suspicious-login notify
- [ ] Force password change on first login
- [ ] Onboarding welcome screen
- [ ] Device list + remote logout
- [ ] Capability-gated route guards
...
**Acceptance:**
- [ ] Capability matrix live + enforced on routes
- [ ] Designations master (15 seed designations)
- [ ] HR + Employee account CRUD; dept CRUD (Admin only)
- [ ] Auto-numbering configurable
- [ ] Master-data table pattern reusable
- [ ] Employee Directory searchable w/ Send Message
- [ ] Profile screens (all roles) w/ photo popup
- [ ] Full seed loaded (1 co / 2 depts / 13 employees)
...
**Acceptance:**
- [ ] Design tokens + brand palette in code
- [ ] Light + dark (both colorful); density control
- [ ] Top bar + role-aware sidebar + mobile nav + breadcrumbs
- [ ] Pinned items engine
- [ ] Component library in packages/ui (button/card/table/badge/dialog/drawer/tooltip/toast/skeleton/empty/palette/shortcut-overlay)
- [ ] Form system (validation/autosave/draft/restore)
- [ ] Filter/sort bar; confirmation dialogs; inline editing; dnd reorder; pagination
- [ ] Keyboard shortcuts (Ctrl+K/N//, Esc, Enter)
...
**Acceptance:**
- [ ] Widget engine (drag/resize/collapse/refresh/lazy/offline/realtime; adaptive sizes)
- [ ] React Grid Layout per-user rearrange; independent loading; dismissible
- [ ] Generic Metric Widget (JSON-fed)
- [ ] Admin/HR/Employee dashboards render (module widgets stubbed then plugged in)
- [ ] Quick Task Assignment widget wired to later module
...
**Acceptance:**
- [ ] Clock in/out + breaks; live timer (continues, amber overtime)
- [ ] Heatmap history + per-day summary
- [ ] Admin company-wide view; HR today view + weekly/monthly graphs
- [ ] Manual correction; overtime + late badge
- [ ] Excel export
- [ ] Reminder scheduler (15-min before / 30-min after)
- [ ] Offline timer + sync (Server-Validation)
...
**Acceptance:**
- [ ] Approval state-machine framework
- [ ] Employee→HR and HR→Admin leave flows
- [ ] History + status badges
- [ ] Holiday calendar view
- [ ] Approval notifications surface in bell + Notification Center
...
**Acceptance:**
- [ ] Project CRUD + team auto-access; sort
- [ ] Task create/assign/priority/due/scope; dependencies; comments; activity log
- [ ] Kanban + list; drag reorder; inline editing
- [ ] QA form builder + submission note
- [ ] Project work timer
- [ ] Recurring tasks
- [ ] Quick Task Assignment wires up (Global Chat notify on completion)
- [ ] Task + project submission/approval cycle
- [ ] Gantt/Timeline view (HR/Admin)
- [ ] Project history; personal task list; saved views/custom columns
...
**Acceptance:**
- [ ] Global/Project/Direct/Group chats over Reverb
- [ ] @mentions; DM read receipts; pin messages; read/unread state
- [ ] Image/file sharing (limits); offline queue
- [ ] Bell + Notification Center + history
- [ ] Announcement board (pin, reactions, dashboard, notify)
- [ ] Quick Notes; complaint/feedback channel
- [ ] Mobile chat UX
...
**Acceptance:**
- [ ] Attendance/project/task/productivity reports (Admin; HR limited)
- [ ] Excel + PDF export
- [ ] Sunday weekly summary email (scheduler)
- [ ] Saved views; filters; virtualized
...
**Acceptance:**
- [ ] Company profile, working hours, holiday calendar, password/session policies, notification prefs, reminder times
...
- [ ] **Capability matrix** — extended each phase (single source: project §18 + per-phase design)
- [ ] **OpenAPI spec** — grown each phase; contract tests on CI
- [ ] **Offline Engine** — each phase's writes routed through it
- [ ] **Seed data** — grown per phase; full by Phase 2
- [ ] **Tests** — feature (Laravel) + component/integration (web) per phase
- [ ] **Deployment** — staging per phase; production + rollback on completion
- [ ] **Backups** — Supabase automated; restore drill before M1 ship
...
- [ ] Add its route(s) to the **bundle budget** (`@next/bundle-analyzer`, First-Load JS ≤200KB gz).
- [ ] Pass **Lighthouse CI** on new routes (LCP≤2.5s, INP≤200ms, CLS≤0.1, FCP≤1.8s).
- [ ] Add **query-count tests** for new list endpoints (≤5 SQL, zero N+1 via `DB::enableQueryLog`).
- [ ] Confirm **indexing** for every new filtered/joined/ordered column (migration checklist).
- [ ] **Virtualize** any new list >100 rows; memoize rows; stable keys.
- [ ] Add **optimistic UI** for safe mutations; per-widget **error boundaries**; skeletons over spinners.
- [ ] Name **frequent workflows** and verify ≤2-click + no-reload targets (esp. Attendance Phase 5).
- [ ] Wire **monitoring** (Sentry perf + web-vitals + Pulse) for the new flows.
- [ ] Record a **performance-notes** section in the archived spec; log any budget **breach** below with owner + remediation plan.
