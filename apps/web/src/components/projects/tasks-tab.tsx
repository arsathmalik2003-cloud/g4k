...
export function TasksTab() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: [...queryKeys.tasks, statusFilter, searchQuery, assigneeFilter, viewMode === "list" ? page : "1", viewMode === "list" ? perPage : 100],
    queryFn: () => {
      ...
      {viewMode === "list" && (
        <div className="space-y-4">
          ...
          <DataTable
            ...
            page={parseInt(page)}
            perPage={perPage}
            totalPages={data?.meta?.last_page || data?.data?.last_page || 1}
