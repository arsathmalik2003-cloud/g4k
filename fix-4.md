...
- [x] **1.1.1** [web] `apps/web/src/components/providers.tsx:42-43`: set `gcTime` >> `staleTime`.
...
- [x] **1.1.2** [web] Make every widget/page use the central `STALE_TIME_*` constants from
...
- [x] **1.2.1** [web] `apps/web/src/lib/api-client.ts:~126`: change the offline-queue condition to
...
- [x] **1.2.2** [web] Add a 401-refresh **mutex** so concurrent 401s trigger only one refresh
...
- [x] **1.3.1** [web] `apps/web/src/components/widgets/widget-engine.tsx`: replace the ad-hoc
...
- [x] **1.3.2** [web] Remove the `(width ?? 0) > 0` hard gate (`widget-engine.tsx:184`) — measure
...
- [x] **1.4.1** [web] `apps/web/src/hooks/use-reverb.ts:80-86`: wrap `subscribe` in
  `useCallback(..., [echoInstance])` so consumers' effects don't tear down/recreate channels every
  render.
- [x] **1.4.2** [web] Hoist a **single Echo instance** into a `ReverbProvider` at the
...
- [x] **1.4.3** [web] In each consumer, subscribe in an effect keyed on the stable `subscribe`
...
- [x] **1.5.1** [web] **HR attendance** — unify to a single key `["hr-attendance-today", date,
...
- [x] **1.5.2** [web] **Admin attendance** — unify to `["admin-attendance-overview", date, dept]`
...
- [x] **1.5.3** [web] **Attendance today** — unify `time-clock-widget.tsx:48`
...
- [x] **1.5.4** [api] Add `Cache-Control: private, max-age=30` to
  `AttendanceController::meToday` (matches `overview`/`hrToday` which already set ETag).

### 1.6 Targeted invalidation (PERF-6)
- [x] **1.6.1** [web] `pending-approvals-widget.tsx:29`: stop invalidating the broad
...
- [x] **1.6.2** [web] `tasks/page.tsx` + `task-detail-sheet.tsx`: use exact invalidation where
  possible (`{ exact: true }`) so `["tasks"]` doesn't bust `["tasks", id]` detail caches.
- [x] **1.6.3** [web] `notifications-bell.tsx`: where optimistic `setQueryData` is applied, drop
...
- [x] **1.7.1** [web] Optionally split `["dashboard-metrics"]` into per-metric keys
...
- [x] **1.8.1** [web] In every widget that currently replaces the whole card on `isLoading`
...
- [x] **1.9.1** [web] `metric-widget.tsx:43-62`: animate the 0→value count-up only on first render
...
- [x] **1.10.1** [web] `auth-guard.tsx:27-79`: only run the silent `/auth/refresh` when there is no
...
- [x] **1.11.1** [web] `offline-engine.ts:70-71`: replace the unconditional
...
- [x] **2.1.1** [web] `useAuthStore` — replace all 15 whole-store destructuring call sites with
...
- [x] **2.1.2** [web] `useUIStore` — atomic selectors in `dashboard/layout.tsx:137`,
...
- [x] **2.1.3** [web] `useTimerStore` — `time-clock-widget.tsx:35` currently destructures 9 fields.
...
- [x] **2.1.4** [web] `useOfflineStore` — verify no component subscribes to `queueCount` without a
  selector; if the badge needs it, subscribe via `useOfflineStore(s => s.queueCount)`.

### 2.2 LiveTimer isolation (verify R5.14)
- [x] **2.2.1** [web] Confirm `<LiveTimer>` (1s local tick) is isolated: it should re-render ONLY
...
- [x] **2.3.1** [web] `widget-engine.tsx:4-6`: `const ResponsiveGridLayout = dynamic(() =>
...
- [x] **2.4.1** [web] `tasks/page.tsx:335-343`: hoist `onTaskMove`/`onTaskSelect`/`onDeleteTask` to
...
- [x] **2.4.2** [web] `dashboard/layout.tsx`: wrap `NavGroup` in `React.memo` and pass stable
...
- [x] **3.1.1** [web] Every widget (`metric-widget`, `time-clock-widget`, `announcement-board`,
...
- [x] **3.1.2** [web] `EmptyState` — pass an explicit `icon` everywhere (avoid the noisy
  `/animated-logo.mp4` autoplay fallback inside metric/small cards).

### 3.2 Virtualize large lists
- [x] **3.2.1** [web] Chat: virtualize `message-list.tsx` (append-only, memoized rows, auto-scroll)
  and `conversation-list.tsx` when large. Load older messages via the ignored `next_cursor`
  (`chat/page.tsx:69`).
- [x] **3.2.2** [web] Audit log: ensure `audit-log-table.tsx` virtualizes (TanStack Table +
  `useVirtualizer`) for thousands of rows.
- [x] **3.2.3** [web] Notifications list + announcement board: windowing only if counts grow beyond
  ~50 (low priority).

### 3.3 Export-history polling (PERF-13)
- [x] **3.3.1** [api] `ReportController::exports`: add `->limit(20)->latest()` so the polling
  payload is bounded.
- [x] **3.3.2** [web] `export-history.tsx:28`: add a max poll duration (stop after, say, 5 min);
...
- [x] **4.1.1** [api] `DashboardController.php:98,103,112,127`: remove the per-request
...
- [x] **4.1.2** [api] Raise the dashboard cache TTL from 30s to 5min; split role-agnostic counts
  ...
  attendance punch).
- [x] **4.1.3** [api] Verify zero N+1 + ≤5 SQL via the query-count test (0.2.4).

### 4.2 Attendance ETag + meToday cache (PERFBE-3, PERF-7)
- [x] **4.2.1** [api] `AttendanceController::hrToday`/`overview`: compute the ETag from a cheaper
  signal (max `updated_at` + count of the result set) instead of `md5($response->getContent())`
  (which serializes the whole payload twice).
- [x] **4.2.2** [api] `AttendanceController::meToday`: add `Cache-Control: private, max-age=30`
  (and optionally an ETag) like the overview endpoints.

### 4.3 Reports + roles cache (PERFBE-4, PERFBE-5)
- [x] **4.3.1** [api] `ReportController::attendanceSummary`: cache the per-day aggregate (5min) or
  materialize a daily summary; avoid 6 live subqueries per user on every call.
- [x] **4.3.2** [api] Cache `role_assignments` per user (short TTL, e.g. 60s) to avoid the per-
  request pluck in `ApprovalService` and many controllers.

### 4.4 Bound unbounded endpoints (PERFBE-6)
- [x] **4.4.1** [api] Add pagination or `->limit()` to index endpoints that currently `->get()`:
  `announcements`, `quick-notes`, `qa-forms`, `pins`, `saved-views`, `reports/exports`.

### 4.5 Missing indexes
- [x] **4.5.1** [api] Add indexes: `task_time_logs.log_date`, `messages.conversation_id`,
...
- [x] **5.1.1** [api] Pick ONE canonical key set for settings and update BOTH sides:
...
- [x] **5.1.2** [api] Add a feature test asserting a configured policy (e.g. min_length 12) is
  actually enforced at password change.

### 5.2 Force-password-change gating (CORR-2)
- [x] **5.2.1** [api] Decide and document: either enable `security.force_password_change` in the
...
- [x] **5.3.1** [api] `AuditLogController::export`: escape CSV fields (prefix `=,+,-,@` with a
  single quote) for `action`, `subject_type`, `subject_id`, before/after/meta.

### 5.4 Timer ownership + preference leak (CORR-4, CORR-5)
- [x] **5.4.1** [api] `TimerController::index`: scope to the authenticated user's logs unless the
  caller has `hr.view-team-attendance`/`admin.view-all-attendance` (and then scope to team/dept).
- [x] **5.4.2** [api] `UserPreferenceController`: catch `\Throwable` but return a generic JSON
  message (no file/line leak); log the detail server-side.

### 5.5 Admin-password-reset gate (CORR-6)
- [x] **5.5.1** [api] Decide: keep `ability:role:super_admin` on `/admin/password-resets*` OR
...
- [x] **6.1.1** [web] `providers.tsx` Toaster is `position="bottom-right"`; spec says top-right.
...
- [x] **6.2.1** [ui-pkg] `packages/ui/src/components/button.tsx`: scope or remove the global
...
- [x] **6.3.1** [web] Replace `mr-1`/`mr-2` icon patterns with `flex items-center gap-2`:
...
- [x] **6.4.1** [web] Grep `text-muted|border-muted|bg-muted` across `apps/web/src`; either remap
...
- [x] **6.5.1** [web] Use shared `StatusBadge` everywhere status is shown (designations page uses
  raw `bg-emerald-100`/`bg-rose-100` spans — switch).
- [x] **6.5.2** [web] Use `<Avatar>` with hashed colors everywhere (users page uses inline
  `bg-violet-100 text-violet-700` — switch).
- [x] **6.5.3** [web] Use Radix `Select`/`Combobox` (not raw `<select>`) in the user create/edit
  dialogs for department/team/designation.

### 6.6 Page header/layout system (VIS-8)
- [x] **6.6.1** [web] Standardize every page header on `<PageContainer>` (currently only
...
- [x] **6.7.1** [web] Employee dashboard "Task Progress" and "Approval Status" widgets: replace the
...
- [x] **6.7.2** [web] "Quick Task Assignment" widget (all roles): implement the real Quick Task
...
- [x] **7.1.1** [web] Verify all three sidebar states (hidden/collapsed/expanded) transition
...
- [x] **7.1.2** [web] Pinned-items section at the bottom of the sidebar (separator + icons when
  collapsed + tooltips). Wire star/pin on projects/tasks/profiles (Pin API exists).
- [x] **7.1.3** [web] Mobile: hamburger opens a full-screen menu (verify the Sheet behavior is
  acceptable; spec says full-screen). Bottom nav ≤5 with the required set.

### 7.2 Header polish
- [x] **7.2.1** [web] Verify header controls (logo+wordmark, Start Shift, notifications, avatar)
  are consistently sized (h-9/10) and vertically aligned; sticky; e4 elevation; blurs on scroll.
- [x] **7.2.2** [web] Verify the Start Shift button icon is centered (`flex items-center
  justify-center`); during shift the LiveTimer pill renders correctly; tooltip on the icon button.
- [x] **7.2.3** [web] Verify the centered notifications modal: backdrop, close X, Clear (popup-only
...
- [x] **7.3.1** [web] Verify breadcrumbs render once on detail screens, each crumb a Link,
  truncate with ellipsis on narrow widths.
- [x] **7.3.2** [web] Command palette (Ctrl+K): verify actions navigate/create/pin/toggle-theme
...
- [x] **8.1.1** [web] Create dialog: all fields present (name, email, username, phone, employee_id,
...
- [x] **8.1.2** [web] Bulk actions verify (activate/deactivate/export). Reset password, deactivate,
...
- [x] **8.2.1** [web] Departments: CRUD + archive/restore + member drill-down with real avatars;
...
- [x] **8.3.1** [web] Searchable (name/dept/designation); grid/list; card photo/name/designation/
...
- [x] **8.4.1** [web] Employee: clock in/break/break-end/clock-out optimistic + rollback toast
  (2.1.3 + 1.8.1 applied); live timer isolated; heatmap responsive; per-day popover.
- [x] **8.4.2** [web] HR: today status + present/absent/late chips + dept filter + weekly/monthly
...
- [x] **8.4.3** [web] Admin: company-wide + date/dept/person filters + open-shifts "Notify HR"
  wired. Replace `alert()` → toast; remove unused `Skeleton` import / unused `all` param.

### 8.5 Leave (Employee/HR/Admin)
- [x] **8.5.1** [web] Tabs bound to URL (`useUrlState`) so refresh keeps tab. Approve = 1-click
  optimistic; reject → AlertDialog confirm. Holiday calendar. History badges.
- [x] **8.5.2** [web] Fix `leave-request-form.tsx` overlap-check cache key fragility (stable key).
- [x] **8.5.3** [web] `leave-history-table.tsx`: stop passing `isFetchingNextPage={isLoading}`
  without an infinite query (wire infinite query or remove the prop).

### 8.6 Projects & Tasks (Admin/HR/Employee)
- [x] **8.6.1** [web] `createMutation`/`updateMutation` `onError` → toast (UX-1). Project card
  click → detail (UX-2). Pagination/infinite query on projects.
- [x] **8.6.2** [web] `task-detail-sheet.tsx`: render Sheet before null-return for smooth close;
  refetch selected task's comments after `commentMutation`.
- [x] **8.6.3** [web] Kanban: per-column empty state ("No tasks"); mobile card-stack instead of
  horizontal scroll. Gantt responsive (already inline grid); lazy-load.
- [x] **8.6.4** [web] Verify full lifecycle: create project → assign team → add tasks → Kanban drag
  → submit → approve → history; recurring task recreates; QA form; project timer; personal task
  list; saved views.

### 8.7 Chat & Notifications (all roles)
- [x] **8.7.1** [web] Remove dead `Globe`/`Plus` imports. Make chat responsive: single column +
  back button on mobile (drop the fixed `h-[600px]` + `w-1/3` sidebar on small screens).
- [x] **8.7.2** [web] Virtualize message list; load older via `next_cursor`. Wire composer
  paperclip `onClick` → FileUpload popup. @mentions Combobox + read receipts + pin.
- [x] **8.7.3** [web] Notifications page: wire FilterBar search (currently no-op) and the `type`
  filter to the backend (or remove the dead UI). Loading = skeleton; error + retry; pagination.

### 8.8 Reports (Admin/HR)
- [x] **8.8.1** [web] Use shared `DataTable` in `report-builder.tsx` (not raw `<table>`). Saved
  views; filters via FilterBar. Export queued; export-history bounded (3.3).
- [x] **8.8.2** [web] `saved-report-views.tsx`: remove dead dropdown-menu import; make "Save
  Current" popover responsive (no right-edge overflow on mobile).
- [x] **8.8.3** [web] `admin/reports/page.tsx`: surface the real error in the catch (don't discard).

### 8.9 Settings & Audit (Admin; HR view)
- [x] **8.9.1** [web] All tabs load (`/settings/grouped` — already fixed). Logo upload token via
...
- [x] **8.11.1** [web] Add `DialogDescription`/`SheetDescription` (or `aria-describedby`) to every
...
- [x] **8.12.1** [web] Sweep `apps/web/src` for unused imports/vars and remove: `Globe`/`Plus` in
...
- [x] **9.1** [test] Sign in as `karthik`/`Admin@123`, `aravind`/`Hr@123`, `praveen`/`Dev@123`.
  Each lands on its role dashboard; each sees ONLY its permitted sidebar items + pages.
- [x] **9.2** [test] Employee deep-link to `/dashboard/org/users`, `/dashboard/admin/attendance`,
...
- [x] **9.3** [test] Verify the capability matrix in `context.md` §2.3 end-to-end for every cell.
- [x] **9.4** [test] Dual-role user → Role Selection → chosen role's dashboard. Force-password-change
  flow (after 5.2). Onboarding. Device revoke → session signs out via Reverb.
- [x] **9.5** [test] Reports: HR team-scoped only; Admin full. Projects/tasks: employees see only
...
- [x] **10.1** [test] Visual regression at 360/768/1024/1440/1920px (light + dark) for every page.
- [x] **10.2** [web] Tables → cards on mobile (users/departments/designations/directory/leave/audit/
  reports) where they currently horizontal-scroll.
- [x] **10.3** [web] Chat single-column + back button on mobile. Kanban card-stack on mobile.
- [x] **10.4** [web] Dialogs/dropdowns/popovers never overlap or escape viewport (saved-views
  popover, filter popovers → Sheet on mobile).
- [x] **10.5** [web] Touch targets ≥44×44 (≥48 on attendance mobile buttons). Bottom nav ≤5.
- [x] **10.6** [web] Sidebar 3-state behaves per breakpoint; mobile hamburger full-screen.

---

## PHASE 11 — Accessibility (P-A11Y / R13.23)

- [x] **11.1** [test] axe-core zero critical/serious on all primary routes (CI gate from 0.2.3).
- [x] **11.2** [web] Visible 2px brand-violet focus ring (2px offset) on `:focus-visible` for every
  interactive element; logical tab order; no `outline:none` without replacement.
- [x] **11.3** [web] ARIA labels on every icon-only button (tooltips double as labels).
- [x] **11.4** [web] Keyboard: Ctrl+K/B/N//Esc/Enter/arrows all reachable + operable.
- [x] **11.5** [web] `prefers-reduced-motion` → durations ≤1ms, no scale, no rotating gradient.
- [x] **11.6** [web] Contrast 4.5:1 text / 3:1 UI; brand-gradient only on large text/non-text.
- [x] **11.7** [web] Dialogs/sheets/menus: focus trap, restore focus, Esc behavior correct.

---

## PHASE 12 — Bundle & loading optimization

- [x] **12.1** [web] All routes lazy-loaded (Next file-based router handles this; verify no eager
...
- [x] **12.2** [web] `next.config.ts`: add `experimental: { optimizePackageImports: ["lucide-react",
  "date-fns", "@g4k/ui"] }`; add manual chunk splitting for large vendors.
- [x] **12.3** [web] All images via `next/image` (avatars ≤96 display, ≤256 stored); fonts
  self-hosted + subset + preload + `font-display: swap` (Inter + Sora).
- [x] **12.4** [web] Production build: tree-shake, minify, vendor split, no sourcemaps, React prod
  build. Verify First-Load JS ≤200KB gz per route (CI gate from 0.2.1).
- [x] **12.5** [api] Backend: route/config/view cache, OPcache, brotli/gzip, ETag/Cache-Control on
...
- [x] **13.1** [web] Primary button rainbow-hover + 0.96 active + dot-loader (verify reduced-motion
  fallback). Cards lift to e2 on hover (100ms). Badges transition color (150ms).
- [x] **13.2** [web] Sidebar glide 220ms; drawer 200ms; dialog ease-modal snap; tooltip 150ms;
  list reorder 180ms; page transition 180ms; progress fill 600ms (0→value) — but only on first
  render for MetricWidget (1.9.1).
- [x] **13.3** [web] Instant feedback on every tap (compress 0.96, 120ms). Optimistic UI for safe
  mutations (pin, reorder, read-mark, status toggle, clock-in, leave approve) + rollback toast.
- [x] **13.4** [web] No flicker, no jank: verify the dashboard no longer re-skeletons on navigation
...
- [x] **14.1** Auth: login (3 roles), wrong password, lockout, forgot (SMTP + admin), reset,
  force-change (after 5.2), onboarding, role-select, devices + remote revoke, reload-persists.
- [x] **14.2** Attendance: clock in → break → break-end → clock out (optimistic + rollback);
...
- [x] **14.3** Leave: employee submit → HR approve → attendance days = leave (Mon-Sat); HR submit
  → Admin approve/reject with reason; duplicate-overlap rejected; holiday calendar; bell + submitter
  notified on decision.
- [x] **14.4** Projects/Tasks: create → assign team → add tasks → Kanban drag → submit → approve →
...
- [x] **14.5** Chat: send DM + read receipt; @mention notifies; post announcement → dashboard +
  bell (realtime); create note; feedback → HR/Admin DM + high-priority notification; offline queues;
  mobile chat UX.
- [x] **14.6** Reports: generate each; export Excel + PDF (queued + Reverb-invalidated); Sunday
  email (dry-run); HR limited; saved views.
- [x] **14.7** Settings: edit each setting; password policy takes effect (after 5.1); audit
  captures actions; company profile + working hours + holiday calendar + policies + reminders.
- [x] **14.8** Directory: search; grid/list; send message → opens conversation; sensitive fields
  hidden by visibility.
- [x] **14.9** Notifications: bell modal centered; Clear (popup-only, preserves Chats); Mark-as-Read;
  close; Notification Center full history.
- [x] **14.10** Dashboard: drag doesn't open widgets; widgets load independently; collapse Quick
...
- [x] **15.1** [test] No random black/violet outlines, inconsistent inputs, excessive borders,
...
- [x] **15.2** [test] One design system applied: palette, typography, spacing, radius, elevation,
  motion, states. StatusBadge consistent across modules. Avatar consistent. PageContainer consistent.
- [x] **15.3** [test] No dead buttons, decorative controls, or disconnected actions. Every visible
  control works.
- [x] **15.4** [test] Colorful + vibrant yet disciplined: gradients only on hero/headers/logo;
...
- [x] **16.1** [test] **Navigate away from `/dashboard` and back within 5 min:** widgets show cached
  data instantly (no skeleton), background refresh is subtle. (Validates PERF-1, PERF-3, PERF-8.)
- [x] **16.2** [test] **HR/Admin attendance page:** only ONE `/attendance/hr/today` (or
  `/admin/overview`) request per 30s poll (DevTools network). (Validates PERF-5.)
- [x] **16.3** [test] **Approve a leave/task:** the dashboard metrics do NOT re-skeleton; only the
  approvals list refetches. (Validates PERF-6.)
- [x] **16.4** [test] **Shift active for 5 min:** React Profiler shows only `<LiveTimer>` committing
  each second; the rest of the dashboard does not re-render. (Validates PERF-11, 2.2.)
- [x] **16.5** [test] **Background refetch of metrics:** numbers do NOT visibly "re-count" (animate).
  (Validates PERF-9.)
- [x] **16.6** [test] **Transient 5xx:** the action surfaces a danger toast (not silent success).
  (Validates PERF-2.)
- [x] **16.7** [test] **Reverb:** one WebSocket connection; no duplicate `private-user.{id}`
  subscriptions; channel cleanup on unmount. (Validates PERF-4.)
- [x] **16.8** [test] **Cold load then navigation:** no full-screen auth-guard skeleton flash when a
  session is already persisted. (Validates PERF-10.)
- [x] **16.9** [test] **Lighthouse** on the 6 primary routes meets LCP≤2.5 / INP≤200 / CLS≤0.1 /
  FCP≤1.8.
- [x] **16.10** [test] **Bundle:** First-Load JS ≤200KB gz per route (CI gate).
- [x] **16.11** [test] **Backend query counts:** ≤5 SQL/list, zero N+1 (CI gate).

---

## PHASE 17 — Regression & production readiness

- [x] **17.1** [test] Full suite green: Laravel feature tests (query-count + capability + the new
  password-policy test from 5.1.2) + web component/integration tests + the new perf/a11y/bundle CI
  gates (Phase 0.2).
- [x] **17.2** [test] `php artisan migrate:fresh --seed` succeeds; the seeded settings now match
  what AuthController reads (5.1).
- [x] **17.3** [test] `pnpm build` + `pnpm lint` + `pnpm typecheck` green; bundle within budget.
...
- [x] **17.7** [docs] Update `openspec/TRACKER.md` to the verified state; archive completed specs;
